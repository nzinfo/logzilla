...
if ($q =~ /^[Ss]/) {
    my $q = &p("Remove LogZilla entries from syslog-ng.conf?", "N");
    if ($q =~ /^[Yy]/) {
        &rm_config_block("/etc/syslog-ng/syslog-ng.conf");
    }
    my $q = &p("Remove sudo entries from /etc/sudoers?", "N");
    if ($q =~ /^[Yy]/) {
        &rm_config_block("/etc/sudoers");
    }
    my $q = &p("Remove /etc/cron.d/logzilla?", "N");
    if ($q =~ /^[Yy]/) {
        unlink("/etc/cron.d/logzilla");
    }
    my $q = &p("Remove AppArmor profile for LogZilla?", "N");
    if ($q =~ /^[Yy]/) {
        &rm_config_block("/etc/apparmor.d/usr.sbin.mysqld");
    }
    my $q = &p("Remove IONCube loader from php.ini?", "N");
    if ($q =~ /^[Yy]/) {
    ...
    my $q = &p("Remove Logrotate.d file for LogZilla?", "N");
    if ($q =~ /^[Yy]/) {
        rm_logrotate();
    }
    my $db_q = &p("Drop $db database?", "N");
    if ($db_q =~ /^[Yy]/) {
        ...
        if ($db_q =~ /^[Nn]/) {
            my $q = &p("Would you like to remove all logs from the $db database?", "N");
            if ($q =~ /^[Yy]/) {
                clean_db();
            }
        }
...
if ($q =~ /^[Aa]/) {
    ...
    my $q = &p("Are you SURE you want to do this?", "N");
    if ($q =~ /^[Yy]/) {
        ...
        &rm_config_block("/etc/apparmor.d/usr.sbin.mysqld");
        print "Removing IONCube loader from php.ini\n";
        my $file ="/etc/php5/apache2/php.ini";
        if (! -e "$file") {
            $q = &p("Please specify the location of your php.ini file:", "$file");
        }
        ...
        print "Removing LogZilla from log rotate.d\nNOTE: YOU MUST MANUALLY DELETE /var/log/logzilla\n";
        rm_logrotate();
        drop_db();
    }
}
