...
TABLE=sph_metrics
FNAME=/tmp/`hostname`_LogZilla_Perf-$(date +%Y.%m.%d).csv
...
mysql -u $dbuser -p$dbpass $db -B -e "SELECT * INTO OUTFILE '/tmp/tempfile.csv' FIELDS TERMINATED BY ',' OPTIONALLY ENCLOSED BY '\"' FROM $TABLE WHERE ts_start >= UNIX_TIMESTAMP(NOW() - INTERVAL 1 MONTH);"
