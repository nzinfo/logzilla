...
# Create Views
    my $sth = $dbh->prepare("
        ...
        (`suppress`.`expire` > now()))) or $dbtable.`msg` REGEXP
        (select `suppress`.`name` from `suppress` where
    ...
    my $sth = $dbh->prepare("
        ...
        (`suppress`.`expire` > now()))))) and
        (not($dbtable.`msg` REGEXP (select `suppress`.`name` from
...
#    $sth->execute;
    ...
    REPLACE INTO cache (name,value,updatetime) VALUES (CONCAT('chart_mpd_',DATE_FORMAT(NOW() - INTERVAL 0 DAY, '%Y-%m-%d_%a')), (SELECT SUM(counter) FROM `$dbtable` WHERE lo BETWEEN DATE_SUB(CONCAT(CURDATE(), ' 00:00:00'), INTERVAL 1 DAY) AND DATE_SUB(CONCAT(CURDATE(), ' 23:59:59'), INTERVAL  1 DAY)),NOW());
    END 
    };
    my $sth = $dbh->prepare("
        $event
