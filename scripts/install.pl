...
sub install_license {
    ...
    print("\n\033[1m\tLicense\n\033[0m");
    print("\n\033[1m========================================\n\n\033[0m\n\n");
    print "If you have already obtained your license from http://www.logzilla.pro, you may enter it now\n";
    my $ok  = &p("Would you like to paste your license data now? (yes/no)", "n");
    if ($ok =~ /[Yy]/) {

        print "Please paste your license text\n'Include *ALL* text (including the BEGIN and END delimiters) so that I can properly extract the license data.\n";
        while (<STDIN>) {
            if (/<licdata>/../<\/licdata>/) {
                next if /^<licdata>/;
                $answer .= $_;
