...
################################################
my @mods = (qw(DBI Date::Calc Term::ReadLine File::Copy Digest::MD5 LWP::Simple File::Spec String::CRC32 MIME::Lite IO::Socket::INET Getopt::Long CHI Net::SNMP Test::mysqld PerlIO::Util Find::Lib MooseX::Params::Validate Test::Deep Test::MockTime Date::Simple Excel::Writer::XLSX));
...
sub make_dbuser {
    ...
    $grant = qq{GRANT DROP, SELECT, INSERT, UPDATE, DELETE, CREATE, REFERENCES, INDEX, ALTER, CREATE TEMPORARY TABLES, LOCK TABLES, CREATE VIEW, SHOW VIEW, EXECUTE, EVENT, TRIGGER ON `$dbname`.* TO '$dbadmin'\@'$localip'  IDENTIFIED BY '$dbadminpw'};
    ...
    $grant = qq{GRANT DROP, SELECT, INSERT, UPDATE, DELETE, CREATE, REFERENCES, INDEX, ALTER, CREATE TEMPORARY TABLES, LOCK TABLES, CREATE VIEW, SHOW VIEW, EXECUTE, EVENT, TRIGGER ON `$dbname`.* TO '$dbadmin'\@'localhost'  IDENTIFIED BY '$dbadminpw'};
...
sub add_syslog_conf {
    ...
    if ( -e $file ) {
        open my $config, '+<', "$file";
        my @arr = <$config>;
        my $sconf = qq{#<lzconfig> BEGIN LogZilla settings
...
my $sconf2 = q{# Windows Events from SNARE
...
destination d_logzilla {
    program("/var/www/logzilla/scripts/logzilla"
        log_fifo_size(1000)
        flush_lines(100)
        flush_timeout(1)
...
log {
    source(s_logzilla);
    # Enable source below if you are sending SNMP Traps to LogZilla
...
sub setup_sudo {
    ...
    $webuser = "apache" if ((!defined $webuser) && ($ostype =~ /Red/));
    $webuser = "www-data" if ((!defined $webuser) && ($ostype =~ /Ubuntu/));
    ...
    if ( $set_sudo =~ /[Yy]/ ) {
        ...
        print "$webuser ALL=NOPASSWD:$lzbase/scripts/hup.pl\n";
        print "$webuser ALL=NOPASSWD:$lzbase/scripts/licadd.pl\n";
        print "$webuser ALL=NOPASSWD:$lzbase/scripts/doimport.sh\n";
        print "$webuser ALL=NOPASSWD:$lzbase/scripts/dorestore.sh\n";
