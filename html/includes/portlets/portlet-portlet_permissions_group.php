...
<thead>
  ...
  <tbody>
    ...
    <tr>
        ...
        <td>
            <select class="sel_portlet_groups" id="sel_portlet_groups">
        <?php
...
$(function(){
        $("#btnPortletGrpPerm").click( function() { 
           ...
           $.get("includes/ajax/json.useradmin.php?action=portlet_group_perm&all="+all+"&"+both, function(data){
                   ...
                   $('#msgbox_br').jGrowl("You haven't selected anything");
           };
        });
        $("#chk_portlet_groups_allusers").click( function() { 
                if ($('#chk_portlet_groups_allusers').attr('checked')) {
                $('#msgbox_br').jGrowl('<font size="2" color="red">Selecting this box will reset group permissions for ALL users (except <?php echo $_SESSION['ADMIN_NAME']?>)</font>');
                };
                });
});
